...
// TextWriter implements [Writer] by writing one human-readable line per log record
// to [TextWriter.Out]: time, level, quoted message, then space-separated key=value
// attributes. For group values, nested keys are prefixed with the parent key and '.'.
// Log level filtering is done by [Config.Level] on the logger, not by TextWriter.
//
// The mutex only wraps the final [io.Writer.Write] on [TextWriter.Out] (after the
...
// NewTextWriter returns a [TextWriter] that writes to out with default time layout [time.RFC3339Nano].
// Set [TextWriter.DurationFormat] or [TextWriter.TimeLayout] on the returned value to customize encoding.
...
func (t *TextWriter) formatAttr(a Attr) []byte {
	return (textFormat{durationFormat: t.durationFormat()}).attrWithPrefix(a, "")
}

// Write formats record as one line: timestamp, level, quoted message, then attributes.
// ctx is unused but kept for the [Writer] interface. It must not retain record after return.
func (t *TextWriter) Write(ctx context.Context, record Record) error {
	...
	if _, err := buf.WriteString(record.Time.Format(t.timeLayout())); err != nil {
		return err
	}
	...
	if _, err := buf.Write(strconv.AppendQuote(nil, record.Message)); err != nil {
		return err
	}
...
// textFormat implements attribute text formatting (key=value, space-separated groups;
// nested keys use dot prefixes). Shared by [TextWriter] and [Value.String].
type textFormat struct {
	durationFormat DurationFormat
}

// formatTextAttr formats an Attr as bytes for text output ([Value.String] path; default duration format).
func formatTextAttr(attr Attr) []byte {
	return (textFormat{durationFormat: DurationFormatGo}).attrWithPrefix(attr, "")
}
